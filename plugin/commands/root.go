...
func Init() {
	...
	u = utils.NewUtils(ui)
	rootCommand = backuprecoveryv1.GetBackupRecoveryV1Command(u)
	...
	cobra.AddTemplateFunc("getTerminalWidth", utils.GetTerminalWidth)
	...
	// "Override" the default version flag to make it translatable.
	rootCommand.Flags().BoolP("version", "v", false, translation.T("version-global-flag"))
...
{{(.LocalFlags.FlagUsagesWrapped getTerminalWidth) | trimTrailingWhitespaces}}{{end}}{{if .HasAvailableInheritedFlags}}

{{printHeader "global-options-header-help-menu"}}:
{{(.InheritedFlags.FlagUsagesWrapped getTerminalWidth) | trimTrailingWhitespaces}}{{end}}{{if .HasHelpSubCommands}}
...
`)
	...
	// the help menu unconditionally. to be more consistent with other ibm plug-ins,
	...
	rootCommand.SetHelpFunc(func(cmd *cobra.Command, args []string) {
		// args are any strings that come after the plug-in root command
		...
		if len(suggestions) > 0 {
			u.Verbose(translation.T("suggestions", map[string]interface{}{
...
func isURLCommand(cmd *cobra.Command) bool {
	return cmd.Name() == "url"
}
...
// the core cli recognizes all of the parent commands due to their existence in the plug-in metadata.
// it extracts only the final word in the command string to pass to this plug-in. cobra needs the
...
func replicateOriginalArgs(maskedArgs string, args []string) []string {
	// plug-ins can run outside of the ibmcloud context, in which case the masked args will be empty
	if maskedArgs == "" {
		return args
	}
	masked := strings.Fields(maskedArgs) // get the args as an array
	return append(masked[1:], args...)   // don't include the first masked arg, it's the plug-in name
}
...
// and make them translatable, to support cli plug-ins in languages other
